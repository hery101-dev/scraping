...
try:
    ...
    df = pd.read_excel('excel_data/company.xlsx')
    ...
    if conn.is_connected():
        ...
        for index, row in df.iterrows():
            ...
            cursor.execute("SELECT COUNT(*) FROM company WHERE company_name = %s", (row['Company Name'],))
            result = cursor.fetchone()

            if result[0] == 0:  # Si le nom de l'entreprise n'existe pas, l'insérer
                cursor.execute("INSERT INTO company (company_name) VALUES (%s)", (row['Company Name'],))
            else:
                print(f"L'entreprise '{row['Company Name']}' existe déjà dans la base de données.")
